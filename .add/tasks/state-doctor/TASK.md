...
- [ADD · open] a "REPORTS instead of aborts" diagnostic must be tested against TYPE-corrupt (not just parse-corrupt) state — the refute-read found an AttributeError path the 6 contracted scenarios missed; the design-for-failure promise only holds with an explicit type-robustness scenario (evidence: test_doctor_reports_not_aborts_on_type_corrupt_state added post-review)
- [TDD · open] a substring assert on a 1-char slug (`assertIn("t", out)`) is vacuous — incidental letters in the PASS line satisfy it; assert the QUOTED form (`"'t'"`) so the test actually pins provenance (evidence: refute-read Finding 2, tightened in this build)
