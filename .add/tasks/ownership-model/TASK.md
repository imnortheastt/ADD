...
- [TDD · open] when a parser NORMALIZES input (extracts a name from `"<...>"`), validate the PARSED value, not the raw arg — a raw `.strip()` check let `--owner "<>"` write a blank name (evidence: review BLOCK on ownership-model; the red test only covered raw whitespace, missing the parsed-empty case).
- [DDD · open] "owner/assignee" (mutable, directive) is a genuinely distinct concept from the "actor stamp" (immutable, historical) even though they share the `{name,email,source}` shape — a new `source:"assigned"` value marks human-typed provenance vs git/os/override resolution (evidence: ownership-model reused the shape but needed the 4th source value to stay honest).
