...
- [ ] machine-state-json   depends-on: none                 — `guide`/`status --json` expose phase · owner · stop; backward-compatible
...
- [ ] `add.py guide --json` and `status --json` emit valid, parseable state incl. owner+stop   (← machine-state-json)
- [ ] the human-text output is unchanged when `--json` is absent (backward compatible)          (← machine-state-json)
