...
- **Done means:** the spec states real user value with no disputed terms and no open assumptions; after release, you have decided what the next loop must address.
- **Apply it:** run the Specify prompt against a real ticket or interview, then confirm or deny every assumption the AI surfaces. If you cannot confirm a rule, it is not ready to build.
